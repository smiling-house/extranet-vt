...
const PartnersBookingpal = (props) => {
...
//By Jaison on 2025-04-21 - END	
	...
	const getAllPartners = async () => {
		setIsLoading(true)
		const partnersResponse = await userRequest.get(`local/partners-bart`,
		...
		} else { console.log("error on reading partners api from shub/local/partners-bart") }
	};
	const getSearchPartners = async () => {
		...
		const partnersResponse = await userRequest.get(`local/partners-bookingpal`,
		{
			params
		});
...
// localStorage.setItem('partnerPropertiesUniqueZipcodes', JSON.stringify(partnerPropertiesUniqueZipcodes));
		...
		console.log("see BP listings for account:", accountId, partner.source);
		localStorage.setItem("bp_listings_partner", JSON.stringify(partner));
		localStorage.setItem("bp_listings_accountId", accountId);
		localStorage.setItem("bp_listings_status", property_status_to_filter || '');

		history.push(PATH_LISTINGS_BOOKINGPAL, { partner, accountId, source: partner.source, status: property_status_to_filter });
								...
								{partners?.map((item, index) => {
									return (
										<tr >
											...
											<td className="VT provider px-4 p-3 text-primary text-decoration-underline cst-cursor">
												<h4 onClick={() => GoToPartnerListings(item, item.accountId, 'Approved')}>{item.approved_properties_count}</h4>
											</td>
											<td className="SH provider px-4 p-3 text-primary text-decoration-underline cst-cursor">
...
export default PartnersBookingpal;
