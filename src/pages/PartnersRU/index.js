...
//import Datatable from "../../components/Datatable"; //COMMENTED OUT AS IT IS NOT USED
...
import "./Admin.scss";
//import ApproveAgent from "./ApproveAgent"; //COMMENTED OUT AS IT IS NOT USED
//import DisApproveAgent from "./DisApproveAgent"; //COMMENTED OUT AS IT IS NOT USED
//import DeletePartner from "./DeleteAgency"; //COMMENTED OUT AS IT IS NOT USED
...
//import ClientOfferLog from "./ClientOfferLog"; //COMMENTED OUT AS IT IS NOT USED
...
const property_status_to_filter_ls = localStorage.getItem('property_status_to_filter_ru'); //ls => local storage
if(property_status_to_filter_ls) {
	property_status_to_filter_ru = property_status_to_filter_ls;
}
    const [filterPropertyStatus, setFilterPropertyStatus] = useState(property_status_to_filter_ru);
    const filterByPropertyStatus = (event) => {
        localStorage.setItem('property_status_to_filter_ru', event.target.value);
        setFilterPropertyStatus(event.target.value);      
    }
    ...
    const getSearchPartners = async () => {
        ...
        const params = (searchInputes.pmName !== '' || searchInputes.accountId !== '') ?
            (searchInputes.pmName !== '') ? {
                ...
                provider:'smilinghouse_channel_api',
                type:'RU',
...
//                 </div>
//             </div>			
//             {/* <div className="page-header" style={{marginLeft: showSideBarMenu ? '250px' : 'unset'}}><img src={menuIcon} style={{'width':'25px'}} className="cst-cursor" onClick={showOrHideSideBarMenu} />&nbsp;{APP_DISPLAY_NAME} : PMs - {agentData.firstName} <span className="cst-cursor" onClick={signOut}>Sign Out</span></div> */}
...
{/*<a class="dropdown-item" href="#" onClick={onAddPartnerVT}><img src={addAdminIcon} /> connect GUESTY PM partner VT = Villa Tracker(TBD IF REQUIRED)</a>*/}
                            </>)}
