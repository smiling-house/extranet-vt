...
const ClientOfferLog = props => {
	...
	const { client, onClose, token, phoneLog, emailLog, NickNameLog } = props;
	...
	let lisingPagingFrom = 1 + pageNumber * constants.PAGING_LISTING_SIZE;
	let lisingPagingTo = (pageNumber + 1) * constants.PAGING_LISTING_SIZE;

	const onChangePage = pageNumber => {
		console.log("page=", pageNumber);
		setPageNumber(pageNumber);
		lisingPagingFrom = 1 + pageNumber * constants.PAGING_LISTING_SIZE;
		lisingPagingTo = (pageNumber + 1) * constants.PAGING_LISTING_SIZE;
		doSearch(pageNumber);
	};
	...
	const getAllClientLogs = async () => {
		const clientLogsResponse = await userRequest.get(`/client-log/getAllLogs`, {
			params: { client_id: client.client_id, limit: constants.PAGING_LISTING_SIZE, skip: lisingPagingFrom - 1 },
		}
		);
	...
	const renderSpecialCollections = row => {
		return (
			<div>
				<img src={
                row?.tags?.indexOf("eventCollection") > -1
                  ? specialEvents
                  : ""
              }
			   className="img-fluid" style={{ marginRight: '5px', width: '25px' }} />
				<img src={
				...
				<img src={
                row?.tags?.indexOf("petCollection") > -1
                  ? dogIcon
                  : ""
              } className="img-fluid" style={{ marginRight: '5px', width: '25px' }} />
			</div>
						...
						<div class="modal-body">
							...
							<div className=" p-2 row">
								<div className="col-md-3">
									...
									<input type="text" className="form-control" placeholder="Nick Name" value={NickNameLog} disabled />
								</div>
								<div className="col-md-3">
									<lable>Email Address</lable>
									<input type="text" className="form-control" placeholder="Email Address" value={emailLog} disabled />
								</div>
								<div className="col-md-3">
							...
							<div className="d-flex justify-content-start">
								<div className="px-4"><b>Travel Details</b></div>
								<div className="px-4"><b>Date:</b> 31.1.2023</div>
								<div className="px-4"><b>Number of guests:</b> 25</div>
								<div className="px-4"><b>Budget:</b> $50,000</div>
								<div className="px-4"><b>Destination:</b> London, UK</div>
